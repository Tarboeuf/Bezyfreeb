...
namespace CommonLib
{
    public class FormUpload : IFormUploadService
    {
        public async Task<string> MultipartFormDataPost(string postUrl, string userAgent, Dictionary<string, object> postParameters, IEnumerable<Tuple<string, string>> headers = null)
        {
            ...
            var retour = await PostForm(postUrl, userAgent, contentType, formData, headers);
            using (StreamReader sr = new StreamReader(retour.GetResponseStream()))
            {
                return sr.ReadToEnd();
            }
        }

        private async Task<HttpWebResponse> PostForm(string postUrl, string userAgent, string contentType, byte[] formData, IEnumerable<Tuple<string, string>> headers = null)
        {
